...
std::unique_ptr<nano::state_block> nano::upgrade_epoch (nano::work_pool & pool_a, nano::ledger & ledger_a, nano::epoch epoch_a)
{
	...
	auto dev_genesis_key = nano::ledger_constants (nano::nano_networks::ananos_dev_network).dev_genesis_key;
...
void nano::system::generate_receive (nano::node & node_a)
{
	...
	{
		auto receive_error (wallet (0)->receive_sync (send_block, nano::ledger_constants (nano::nano_networks::ananos_dev_network).genesis_account, std::numeric_limits<nano::uint128_t>::max ()));
		(void)receive_error;
	}
}
